from tkinter import *
import tkinter.messagebox as msgbox  # 메세지 박스 이용시 필요

root = Tk()
root.title("hwankko GUI")  # 제목
root.geometry("640x480")  # 창크기: 가로 x 세로

# 메시지 박스는 동작에 대한 팝업창으로 사용
# ex) 동작을 실행할 수 없습니다.

# 기차 예매 시스템이라고 가정
def info():
    msgbox.showinfo("알림", "정상적으로 예매 완료 되었습니다.") # showinfo : 정보를 알려줌(타이틀, 메세지)
...
def okcancel():
    msgbox.askokcancel("확인 / 취소","해당좌석은 유아 동반석입니다. 예매하시겠습니까?") # 사용자에게 ok 누를건지 cancel 누를건지 물어봄
